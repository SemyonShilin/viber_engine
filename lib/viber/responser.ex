defmodule Engine.Viber.Responser do
  use Agala.Bot.Responser

  defp create_body(%Agala.Conn{response: %{payload: %{body: body}}})
       when is_bitstring(body) or is_tuple(body),
       do: body

  defp create_body(%Agala.Conn{response: %{payload: %{body: body}}}) when is_map(body),
    do: body |> Poison.encode!()

  defp create_body(_), do: ""

  defp create_url(%Agala.Conn{response: %{payload: %{url: url}}}, bot_params)
